...
import com.hananrh.kronos.extensions.json.annotations.RemoteIntValue
import com.hananrh.kronos.extensions.json.annotations.RemoteStringValue
import kotlin.reflect.KClass

object RemoteValueEnumUtils {

	fun <T : Enum<T>> getIntEnumConst(
		enumClass: KClass<T>,
		remoteValue: Int
	) =
		getEnumConst(
	...
	fun <T : Enum<T>> getStringEnumConst(
		enumClass: KClass<T>,
		remoteValue: String
	) =
		getEnumConst(
			enumClass, RemoteStringValue::class.java, remoteValue.lowercase()
		) {
			value.lowercase()
		}

	fun <T : Enum<T>> getIntEnumRemoteValue(
		enumClass: KClass<T>,
		enumConstant: T
	) =
		getEnumRemoteValue(
	...
	fun <T : Enum<T>> getStringEnumRemoteValue(
		enumClass: KClass<T>,
		enumConstant: T
	) =
		getEnumRemoteValue(
	...
	private fun <A : Annotation, T : Enum<T>, S> getEnumRemoteValue(
		enumClass: KClass<T>,
	...
	@Suppress("UNCHECKED_CAST")
	private fun <A : Annotation, T : Enum<T>, S> getEnumConst(
		enumClass: KClass<T>,
		annotationClass: Class<A>,
		remoteValue: S,
		resolver: A.() -> S
